module UsersHelper
  FORM_TITLES = {
    'users' => {
      'new' => { title: 'Create your GitHub user', btn_text: 'Create user!' },
      'edit' => { title: 'Edit user', btn_text: 'Update' }
    }
  }.freeze

  def users_list
    if @users.instance_of?(String)
  ...
  def create_or_edit_title
    tag.h3(FORM_TITLES.dig(controller_name, action_name, :title).to_s, class: 'text-center')
  end

  def form_button_text(f)
    f.submit(FORM_TITLES.dig(controller_name, action_name, :btn_text).to_s, class: 'btn btn-outline-success mt-2')
  end

  def user_page_btns
    link_to('Update GH info', update_gh_info_path(@user),
            class: 'btn btn-outline-success',
            remote: true,
            data: { confirm: 'This action may overwrite user data. Do you really want to proceed?' }) +
